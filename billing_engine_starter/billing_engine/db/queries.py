...
def select_subscriptions_due_for_billing(conn, billing_date):
    return select_due_subscriptions(conn, billing_date)

def select_plan_tiers_for_plan(conn, plan_id):
    return conn.execute(
        "SELECT * FROM plan_tiers WHERE plan_id = ?",
        (plan_id,)
    ).fetchall()